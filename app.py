...
# 2. รายชื่อและสี (อ้างอิง Master Code)
...
# 5. Sidebar (จุดที่มีระบบ Auto-Progress)
with st.sidebar:
    ...
    # --- ฟังก์ชัน Auto-Progress Logic ---
    status_options = ["Planning", "In Progress", "Completed", "Delayed"]
    
    # 1. เลือก Status ก่อน
    current_status = st.selectbox("Status", status_options, 
                                  index=status_options.index(val['Status']) if val is not None else 0)
    
    # 2. กำหนดเปอร์เซ็นต์เริ่มต้นตาม Status
    if st.session_state.edit_mode and val is not None:
        auto_val = int(val['Progress']) # ถ้าโหมดแก้ ให้ใช้ค่าเดิมก่อน
    else:
        auto_map = {"Planning": 0, "In Progress": 50, "Completed": 100, "Delayed": 25}
        auto_val = auto_map.get(current_status, 0)

    # 3. แสดงแถบ Progress (ที่เปลี่ยนตาม Status อัตโนมัติ)
    progress = st.slider("Progress (%)", 0, 100, auto_val)
    ...
    with st.form("action_form", clear_on_submit=True):
        ...
        c_p = st.columns(2)
        sales_p = c_p[0].selectbox("Sales PIC", SALES_LIST, index=SALES_LIST.index(val['Sales PIC']) if val is not None else 0)
        eng_p = c_p[1].selectbox("Engineer PIC", ENG_LIST, index=ENG_LIST.index(val['Eng PIC']) if val is not None else 0)
        
        c_s = st.columns(2)
        priority = c_s[0].selectbox("Priority", ["High", "Medium", "Low"], index=["High", "Medium", "Low"].index(val['Priority']) if val is not None else 1)
        p_status = c_s[1].selectbox("Project Status", ["P0", "P1", "P2", "P3"], index=["P0", "P1", "P2", "P3"].index(val['Project Status']) if val is not None else 1)
        
        c_d = st.columns(2)
        start_d = c_d[0].date_input("Start Date", value=val['Start Date'] if val is not None else datetime.now().date())
        end_d = c_d[1].date_input("End Date", value=val['End Date'] if val is not None else datetime.now().date())

        if st.form_submit_button("💾 บันทึกข้อมูล"):
            new_row = {"Dept": dept, "Activity": activity, "Sales PIC": sales_p, "Eng PIC": eng_p, "Status": current_status, "Progress": progress, "Start Date": start_d, "End Date": end_d, "Priority": priority, "Project Status": p_status}
            if st.session_state.edit_mode:
...
# 6. กราฟและรายละเอียด (เหมือนเดิม)
if not df.empty:
    # --- ระบบ Filter ---
